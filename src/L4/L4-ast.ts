...
export const parseTraceExp: (params: Sexp[]) => Result<TraceExp> = 
    (params) => 
    params.length !== 1 ? makeFailure("Expression not of the form (TraceExp id)") :
    parseGoodTrace(first(params));

export const parseGoodTrace = (moni : Sexp) : Result<TraceExp> => 
    !isIdentifier(moni) ? makeFailure("Expression not an identifier") :
    makeOk(makeTraceExp(makeVarRef(moni)));
