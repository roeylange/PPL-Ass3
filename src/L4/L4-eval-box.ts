...
const evalTraceExp = (exp: TraceExp, env: Env): Result<void> =>
    ...
    isClosure(getFBindingVal(bdg)) ?        //getFB = Unbox
        makeOk(setFBinding(bdg, makeTracedClosure(getFBindingVal(bdg) as Closure, exp.var.var))) :  //set=setBox
...
const applyTracedClosure = (proc: TracedClosure, args: Value[]): Result<Value> => {
    count+=1;   //may be a problem?
    printPreTrace(proc.name , args, count);
    const vars = map((v: VarDecl) => v.var, proc.closure.params);  //from line 78
    return bind(
        evalSequence(proc.closure.body, makeExtEnv(vars, args, proc.closure.env))   //line 79
        , (x:Value) => {
            printPostTrace(x,count);
            count-=1;       //may be a problem?
            return makeOk(x);
        }
    )
